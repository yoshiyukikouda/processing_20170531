...
void setup() {
  ...
  for (int i = 0; i < movers.length; i++) {
    movers[i] = new Mover(random(0.1, 4), 0, 0, random(-300, -50), random(0, 255), random(0, 255), random(0, 255), 255);
  }
}
...
void draw() { 
  // 背景描写
  background(255);
  drawBackground(  0,   0,   0,   0,   0,   0,  50,  true, false);
  drawBackground(200,   0,   0,   0, 255,   0,  50, false, false);
  drawBackground(400,   0,   0, 255, 255,   0, 100, false,  true);
...
* @param : alpha  透過度
*/
void drawBackground(int x, int y, int z, int r, int g, int b, int alpha, boolean isLeft, boolean isRight) {
  // 図形の色、線
  stroke(r-50, g-50, b-50, alpha);
  strokeWeight(1);
  fill(r, g, b, alpha);
...
class Mover {
  ...
  */
  void checkEdges() {
    ...
    // Z軸方向で壁に到達した場合
    if (location.z > 0 + objSize) {
      velocity.z *= -1;
      location.z = 0 + objSize;
